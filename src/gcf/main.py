...
##### TODO: Remove, replaced by read_file_from_gcs
# # Deprecated
...
# ### TODO: Remove as we no longer write directly to GCS
...
def annotate_gcs(cloud_event):
    ...
    image_bytes = read_file_from_gcs(src_bucket,image_file_name)
    vqa_results = vqa(image_bytes,default_question,default_number_of_results)
    # We put the result into list
    # This is done because this one is event driven 
    payload_list = [vqa_results]
    pub_sub_write(project_id,topic_id,payload_list)

    ### TODO: Remove as no longer needed if using VQA 
